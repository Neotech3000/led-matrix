...
- **Groups:** click **New group**, name it (for example Desk mix), and it appears in the bubble row immediately — no restart. Click the **name** to open/view that group (library filters to its ids). Click **+** on the bubble to enter selection mode without opening it — the catalog stays on All/type/search so you can tap many cards (or card + / heart) to add. Hint: “Adding to Desk mix — tap cards to add”. Click **+** again or Escape to leave; click the name while adding to review the group. Each bubble has its own **Random** that cycles only that group (10–30s, both wells, skip sketch/sand). Top-bar **Random** still walks the whole catalog. Group random and catalog Random are mutually exclusive; assigning a card by hand turns that shuffle off.
